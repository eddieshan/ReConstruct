...
//                    index <- index + 2
    ...
    let getBitmap (buffer: int[,]) sliceParams =
        ...
        let bitmap = new WriteableBitmap(sliceParams.Dimensions.Columns, sliceParams.Dimensions.Rows, 96.0, 96.0, PixelFormats.Pbgra32, null)

        let rectangle = new Int32Rect(0, 0, sliceParams.Dimensions.Columns, sliceParams.Dimensions.Rows)
        let stride = (sliceParams.Dimensions.Columns * bitmap.Format.BitsPerPixel) / 8
        bitmap.WritePixels(rectangle, imageBuffer, stride, 0)
...
module Cat =
    ...
    let slice (buffer, dicomTree) =
        ...
        let getBitmap() = 
            let bitmap = sliceParams |> Hounsfield.getBitmap hounsfieldImage
            bitmap.Freeze()
            bitmap :> BitmapSource
